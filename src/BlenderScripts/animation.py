...
def animate_tm(extobj, newtransform):
	...
	if type(extobj) == bpy.types.Object:
		...
		for i in range(0, 3):
			if curve_pos[i] == None:
				curve_pos[i] = action.fcurves.new(curvepath_pos, index=i, action_group=bone_name)
			curve_pos[i].keyframe_points.insert(frame, pos[i])
		for i in range(0, 4):
			if curve_rot[i] == None:
				curve_rot[i] = action.fcurves.new(curvepath_rot, index=i, action_group=bone_name)
			curve_rot[i].keyframe_points.insert(frame, rot[i])
	else:
