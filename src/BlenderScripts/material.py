...
def set_diffuse_map_filename(mat, filename):
	...
	for i in range(0, len(slots)):
		slots.clear(i)
	slot = mat.texture_paint_slots.create(0)
...
def set_diffuse_color(mat, clr, alpha):
	mat.diffuse_color = (clr[0], clr[1], clr[2], alpha)
...
def find_image_in_material_slots(mat):
	...
	if (mat.node_tree == None):
		return None
	bsdf = mat.node_tree.nodes["Principled BSDF"]
	for node_links in bsdf.inputs['Base Color'].links:
		return node_links.from_node.image
	return None
