...
def set_parent(extobj, extnewparent):
	if type(extobj) == bpy.types.Object:
		...
		if extnewparent == None:
			...
			bone = arm.bones[parent_bone_name]
			bone.select = True
...
def is_visible(extobj):
	if type(extobj) == bpy.types.Object:
		return not(extobj.hide_viewport)
	return False

# Sets if an object is visible (in Editor).	
def show(extobj, show):
	if type(extobj) == bpy.types.Object:
		extobj.hide_viewport = not(show)
...
def is_selected(extobj):
	if type(extobj) == bpy.types.Object:
		...
		bone_name = extobj[1]
		arm = obj.data
		return (obj.select_get() and arm.bones[bone_name].select)
	else:
